        ...
        response.json().then(function (data) {
            ...
            for (var j = 0; j < data.results.length; j++)
            {
                var row = `<tr onclick="loadDivsionDetail('${data.results[j].id}')">
                    ...
                    <button type="button" data-toggle="modal" data-target="#edit" class="update btn btn-warning btn-sm"><span class="glyphicon glyphicon-pencil"></span></button>
        ...
        .then(function (response){
            ...
            for (var j = 0; j < data.results.length; j++)
            {
                ...
                var row = `<tr onclick="loadDivsionDetail('${data.results[j].id}')">
                    ...
                    <td>${data.results[j].Ten_TV}</td>
                    <td style = "width: 130px;">
                    <button type="button" data-toggle="modal" data-target="#edit" class="update btn btn-warning btn-sm"></button>
                    <button type="button" data-toggle="modal" data-target="#delete" class="delete btn btn-danger btn-sm"></button>
                    </td>
                </tr>`
                idShow += 1
                if (j == 0) {
