function loadFamilyDetail(x) {
	console.log(x);
	localStorage.setItem('idFamily', x);
	window.location.replace('familyDetail.html');
}
const familyData = 'http://134.209.106.33:8888/v1/familia?limit=20&&page=1';
	...
	.then(function (response) {
		response.json().then(function (data) {
			...
			for (var j = 0; j < data.results.length; j++) {
				var row = `<tr onclick="loadFamilyDetail('${data.results[j].id}')">
                    <td>${j + 1}</td>
			...
			function renderdivision(page) {
				var familyDataPage = 'http://134.209.106.33:8888/v1/familia?limit=20&&page=' + page;
				fetch(familyDataPage, requestOptions).then(function (response) {
					response.json().then(function (data) {
						var table = document.getElementById('familyBody');
