...
const searchFunctionFamily = (id) => {
	...
	if (input.value.length > 0) {
		switch (id) {
			case 'searchFamily': {
				fetch(
					`http://134.209.106.33:8888/v1/familia/searchFamilia/${input.value}`,
					requestOptions,
				).then((response) => {
					response.json().then((data) => {
						var table = document.getElementById('familyBody');
						table.innerHTML = '';
						for (var j = 0; j < data.familias.length; j++) {
							var row = `<tr onclick="loadFamilyById('${data.familias[j].id}')">
                    <td>${j + 1}</td>
                    <td>${data.familias[j].Ten_KH}</td>
                    <td>${data.familias[j].Ten_TV}</td>
                	</tr>`;
							table.innerHTML += row;
						}
					});
